...
retsly.ready(function(){
  ...
  retsly.get('/api/v1/test_sf/listings.json', {limit: 10}, function(res){
    if (!res.success) throw new Error(res.bundle);
    console.log(res);
  });
});
},{"retsly-js-sdk":2}],2:[function(require,module,exports){
/**
 * Dependencies
 */
var emitter = require('component-emitter');
var store = require('component-cookie');
...
function Retsly (client_id, token, options) {
  if (!token) {
    var tokenErrMsg = 'You must provide a browser token';
    tokenErrMsg += ' - ie: new Retsly(\'client_id\', \'token\');';
    throw new Error(tokenErrMsg);
  }

  if (!client_id) {
    var clientErrMsg = 'You must provide a client_id';
    clientErrMsg += ' - ie: new Retsly(\'client_id\', \'token\');';
...
Retsly.create = function create (client, token, opts) {
  ...
  if (!s && !client) {
    throw new Error('call Retsly.create() with client id and options');
  }
...
Retsly.prototype.connect = function(err, rsid) {
  if (err) return this.ready(err);

  // only attempt to connect 3 times then continue on
...
Retsly.prototype.session = function(cb) {
  ...
  if (!token) return cb({ success: false, status: 401, bundle: 'No token set'});
  ...
  ajax({
    ...
    crossDomain : true,
    error: function(xhr) {
      var errMsg = 'Could not set Retsly session';
      debug(errMsg);
      cb(new Error(errMsg));
    }.bind(this),
...
Retsly.prototype.ready = function(cb) {
  var stack = this.__init_stack;

  // handle errors passed to ready
  if (cb && typeof cb === 'object' && stack.length) {
    return stack[stack.length - 1](cb);
  }

  // call all functions in the stack
  if (cb) stack.push(cb);
  else stack.forEach(function(c) {
...
Retsly.prototype.request = function(method, url, query, cb) {
  ...
  var token = this.getUserToken() || this.getAppToken();
  if (!token) return cb({ success: false, status: 401, bundle: 'No token set'});
  ...
  ajax({
    ...
    error: function(xhr, type, settings) {
      ...
      if (!xhr.responseText) self.emit('ServerUnavailable');
...
var getOrigin = Retsly.prototype.getOrigin = function () {
  ...
  origin += (document.location.port ? (':' + document.location.port) : '');
...
},{"component-ajax":3,"component-cookie":5,"component-emitter":9,"extend":10,"qs":11}],3:[function(require,module,exports){
var type
try {
